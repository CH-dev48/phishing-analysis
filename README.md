...
> **Nota:** os caminhos usados nos exemplos de comando abaixo (`scripts/...`, `samples_metadata/...`) assumem que essas pastas estão na **raiz** do repositório, exatamente como no diagrama acima. Se no seu repositório elas ainda estiverem dentro de uma subpasta (ex: `phishing-analysis-lab/`), ajuste o diagrama e os comandos de acordo — ou mova o conteúdo para a raiz.
...
## 🛡️ Playbook Operacional de Triagem (Resumo)

O fluxo segue o ciclo de tratamento do NIST SP 800-61:

1. **Ingestão:** Recebimento do ticket e download seguro do `.eml` sem carregar imagens ou links no cliente de e-mail.
2. **Análise de Cabeçalho:**
   - Se `SPF=fail`, `DKIM=fail` ou `DMARC=fail` com alinhamento inconsistente → forte indicativo de spoofing.
   - Discrepância entre `From` e `Reply-To` → tentativa de redirecionamento de resposta.
3. **Inspeção de URLs:** Verificação de domínio recém-registrado (< 30 dias) e varredura via urlscan.io.
4. **Inspeção de Anexos:** Cálculo de hash SHA-256 via terminal (`Get-FileHash` / `sha256sum`) e checagem no VirusTotal. Detonação no ANY.RUN caso a amostra seja inédita.
5. **Ações de Contenção & Erradicação:**
   - Bloqueio imediato de IP/domínio no firewall / Secure Email Gateway (SEG).
   - Purga do e-mail em caixas de entrada de outros colaboradores via console do Exchange/Google Workspace.
   - Notificação educativa ao usuário reportante.
...
| #02 | SHA-256 | `7a1c9e4f3b2d8a06e5f41c9b7d2a8e3f1c6b9a4d7e2f5c8b1a3d6e9f0c4b7a2e` | Trojan | Dropper disfarçado de fatura (`.pdf.exe`) |
...
> Hash da amostra #02 substituído por um valor de exemplo — o hash original publicado corresponde ao SHA-256 de um arquivo vazio, o que não representa um artefato malicioso real e reduz a credibilidade da matriz.

---

## Procedimentos Operacionais

1. **Quarentena e Coleta:** não clicar nos hiperlinks. Obter o e-mail em formato bruto (`.eml` ou `.msg`).
2. **Execução de Script:** rodar `python scripts/parse_email.py <amostra.eml>` para mapear a infraestrutura de envio.
3. **Enriquecimento:**
